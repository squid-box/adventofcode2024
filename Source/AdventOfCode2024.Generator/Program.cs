namespace AdventOfCode2024.Generator;
    ...
    private static void CreateDailyProblemCodeFiles(int year)
    {
        ...
        try
        {
            for (var day = 1; day < 26; day++)
            {
                ...
                File.WriteAllText(Path.Combine(pathToProblemFile), string.Format("namespace AdventOfCode{2}.Problems{0}{{{0}    /// <summary>{0}    /// Solution for <a href=\"https://adventofcode.com/{2}/day/{1}\">Day {1}</a>.{0}    /// </summary>{0}    public class Problem{1}(InputDownloader inputDownloader) : ProblemBase({1}, inputDownloader){0}    {{{0}        /// <inheritdoc />{0}        protected override object SolvePartOne(){0}        {{{0}            return \"Unsolved\";{0}        }}{0}{0}        /// <inheritdoc />{0}        protected override object SolvePartTwo(){0}        {{{0}            return \"Unsolved\";{0}        }}{0}    }}{0}}}{0}", Environment.NewLine, day, year));
